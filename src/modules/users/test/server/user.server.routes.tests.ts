...
describe("User CRUD tests", function () {
  ...
  before(function (done: any) {
    ...
    mongoose.loadModels();
    const app = new Express(env).init(connection);
  ...
  it("should be able to register a new user", function (done) {
    ...
    agent.post("/api/auth/signup")
      .send(_user)
      .expect(200)
      .end(function (signupErr: any, signupRes: any) {
  ...
  it("should be able to login with username successfully and logout successfully", function (done) {
    ...
    agent.post("/api/auth/signin")
      .send(credentials)
      .expect(200)
      .end(function (signinErr: any, signinRes: any) {
        // Handle signin error
        if (signinErr) {
        ...
        agent.get("/api/auth/signout")
          .expect(302)
          .end(function (signoutErr: any, signoutRes: any) {
            if (signoutErr) {
  ...
  it("should not be able to retrieve a list of users if not admin", function (done) {
    agent.post("/api/auth/signin")
      ...
      .end(function (signinErr: any, signinRes: any) {
        ...
        agent.get("/api/users")
          .expect(403)
          .end(function (usersGetErr: any, usersGetRes: any) {
            if (usersGetErr) {
      ...
      agent.post("/api/auth/signin")
        ...
        .end(function (signinErr: any, signinRes: any) {
          ...
          agent.get("/api/users/" + user._id)
            .expect(200)
            .end(function (userInfoErr: any, userInfoRes: any) {
              if (userInfoErr) {
        ...
        .end(function (signinErr: any, signinRes: any) {
          ...
          agent.put("/api/users/" + user._id)
            ...
            .end(function (userInfoErr: any, userInfoRes: any) {
              ...
              userInfoRes.body.should.be.instanceof(Object);
              userInfoRes.body.fullName.should.be.equal("admin_update_first");
      ...
      agent.post("/api/auth/forgot")
        ...
        .end(function (err: any, res: any) {
          ...
          User.findOne({ username: user.username.toLowerCase() }, function (err, userRes) {
            should.exist(userRes.resetPasswordToken);
            should.exist(userRes.resetPasswordExpires);
            // res.body.message.should.be.equal("Email Sent");
            return done();
          });
        });
    });
        ...
        .end(function (signinErr: any, signinRes: any) {
          ...
          agent.put("/api/users")
            ...
            .end(function (userInfoErr: any, userInfoRes: any) {
              ...
              userInfoRes.body.should.be.instanceof(Object);
              userInfoRes.body.fullName.should.be.equal("user_update_first");
              userInfoRes.body.roles.should.be.instanceof(Array).and.have.lengthOf(1);
